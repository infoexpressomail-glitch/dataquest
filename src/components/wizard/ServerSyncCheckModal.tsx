...
export const ServerSyncCheckModal: React.FC<ServerSyncCheckModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="relative w-full max-w-2xl rounded-2xl border border-slate-800 bg-[#0d0e15] shadow-2xl p-6 sm:p-7 space-y-6 animate-in fade-in zoom-in-95 duration-200">
        
        {/* Close button */}
        <button
          onClick={onClose}
          disabled={step === 'syncing' || step === 'uploading'}
          className="absolute right-4 top-4 rounded-lg p-1 text-slate-400 hover:bg-slate-800 hover:text-white transition disabled:opacity-50"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="flex items-start gap-4">
          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-blue-500/10 border border-blue-500/30 text-blue-400">
        ...
        <div className="rounded-xl border border-slate-800/80 bg-slate-900/50 p-4 space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/60 pb-2.5">
            <div>
              <span className="text-[10px] font-medium text-slate-400">Pesquisa em Andamento:</span>
              <div className="text-sm font-bold text-white">{survey.nome}</div>
            </div>
            <div className="flex items-center gap-2">
              <span className="font-mono text-xs text-slate-400">{survey.codigo}</span>
              <span className="rounded bg-slate-800 border border-slate-700 px-2 py-0.5 text-[11px] font-semibold text-slate-200">
                v{survey.versao}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="rounded-lg bg-slate-800/40 p-2 border border-slate-800">
        ...
        {!effectiveOnline && (
          <div className="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3.5 text-xs text-amber-300">
        ...
        {step === 'prompt' && (
          <div className="space-y-4">
            <div className="rounded-xl border border-slate-800 bg-[#12131c] p-4 text-xs space-y-2.5">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-500 px-5 py-2.5 text-xs font-bold text-white transition shadow-lg shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        {step === 'syncing' && (
          <div className="py-8 text-center space-y-4">
            <div className="flex justify-center">
              <div className="h-12 w-12 rounded-full border-2 border-blue-500/20 border-t-blue-500 animate-spin flex items-center justify-center">
                <Server className="h-5 w-5 text-blue-400 animate-pulse" />
              </div>
            </div>
            <div className="space-y-1">
              <h4 className="text-sm font-bold text-white">Sincronizando com o Servidor Central...</h4>
        ...
        {step === 'authorized' && (
          <div className="space-y-4">
            <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 space-y-2.5">
              ...
              {authorizedToken && (
                <div className="mt-2 flex items-center gap-2 rounded-lg bg-emerald-950/60 border border-emerald-500/20 px-3 py-1.5 font-mono text-[11px] text-emerald-300">
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 px-5 py-2.5 text-xs font-bold text-white transition shadow-lg shadow-emerald-500/20"
                >
        ...
        {step === 'uploading' && (
          <div className="py-8 text-center space-y-4">
            <div className="flex justify-center">
              <div className="h-12 w-12 rounded-full border-2 border-emerald-500/20 border-t-emerald-500 animate-spin flex items-center justify-center">
                <ArrowUpCircle className="h-5 w-5 text-emerald-400 animate-pulse" />
              </div>
            </div>
            <div className="space-y-1">
              <h4 className="text-sm font-bold text-white">Subindo Alterações para o Servidor...</h4>
              <p className="text-xs text-slate-400">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              <button
                type="button"
                onClick={handleExecuteSync}
                className="flex items-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-500 px-5 py-2.5 text-xs font-bold text-white transition"
              >
                <RefreshCw className="h-4 w-4" />
