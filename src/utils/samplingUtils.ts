...
export function calculateSampleSize(
  ...
  const formulaAplicada = isFinite
    ? `n = [Z² · p(1-p) / e²] ÷ [1 + (n₀ - 1) / N] = [(${z.toFixed(2)})² · ${variance} / (${eDec.toFixed(4)})²] com N=${population?.toLocaleString('pt-BR')} => ${nFinal} coletas`
    : `n = [Z² · p(1-p)] / e² = [(${z.toFixed(2)})² · 0.25] / (${eDec.toFixed(4)})² => ${nFinal} coletas`;
